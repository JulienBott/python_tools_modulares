...
class entry_propio(gui_tkinter_widgets):
    ...
    def __exit_entry(self, tipo_validacion: Literal["definida_en_la_clase", "definida_por_el_usuario"],  event = None):
        ...
        try:
            ...
            #############################################################################
            # CASO 2 - validadacion personalizada por el usuario
            #############################################################################
            elif tipo_validacion == "definida_por_el_usuario":
                
                if isinstance(self.funcion_validacion_personalizada, str) and self.self_clase_gui_donde_call_rutina is not None:
                    rutina_objeto = getattr(self.self_clase_gui_donde_call_rutina, self.funcion_validacion_personalizada, None)
                else:
                ...
                if callable(rutina_objeto):
                    ...
                    resultado_funcion_validacion_personalizada_ = rutina_objeto(valor_entry)

                    if resultado_funcion_validacion_personalizada_ == self.resultado_funcion_validacion_personalizada_para_bloquear_exit:
